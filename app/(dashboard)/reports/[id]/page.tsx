...
export default async function ReportDetailPage({
  ...
  const loanTypes: string[] = Array.isArray(extracted?.loan_types)
    ? extracted.loan_types
    : [];
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="truncate text-xl font-semibold">
            {extracted?.customer_name ?? report.file_name}
          </h1>
        ...
        {analysis && (
          <RiskBadge category={analysis.risk_category} size="lg" />
        )}
      </div>
              ...
              <CardContent>
                <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                  ...
                  <DataItem
                    label="Age"
                    value={
                      extracted.age != null ? `${extracted.age} yrs` : "—"
                    }
                  />
                  <DataItem
                    label="Occupation"
                  ...
                  <DataItem
                    ...
                    value={
                      extracted.monthly_income != null
                        ? formatCurrency(extracted.monthly_income)
                        : "—"
                    }
                  />
                </dl>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                  <DataItem
                    label="Active Loans"
                    value={String(extracted.active_loans ?? 0)}
                  />
                  <DataItem
                    label="Outstanding Balance"
                    value={formatCurrency(extracted.outstanding_balance ?? 0)}
                  />
                  <DataItem
                    label="Monthly EMI"
                    value={
                      extracted.emi_obligations
                        ? formatCurrency(extracted.emi_obligations)
                        : "—"
                    }
                  />
                </dl>
              ...
              <CardContent>
                <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                  <DataItem
                    ...
                    value={String(extracted.missed_payments ?? 0)}
                    highlight={
                      (extracted.missed_payments ?? 0) > 0 ? "danger" : "success"
                    }
                  />
                  <DataItem
                    label="Credit Utilization"
                    value={
                      extracted.credit_utilization != null
                        ? `${extracted.credit_utilization}%`
                        : "—"
                    }
                    highlight={
                      extracted.credit_utilization == null
                        ? undefined
                        : extracted.credit_utilization > 60
                        ? "danger"
                        : extracted.credit_utilization > 30
                        ? "warning"
                        : "success"
                    }
                  />
                  <DataItem
                    label="Account Age"
                    value={
                      extracted.account_age_months != null
                  ...
                  <DataItem
                    ...
                    value={String(extracted.hard_inquiries ?? 0)}
                    highlight={
                      (extracted.hard_inquiries ?? 0) > 4
                        ? "danger"
                        : (extracted.hard_inquiries ?? 0) > 2
                        ? "warning"
                        : "success"
                    }
                  />
              ...
              <CardContent className="flex min-h-[200px] items-center justify-center py-10">
                <p className="text-center text-sm text-muted-foreground">
                  {report.status === "processing"
                    ...
                    ? `Analysis failed: ${report.error_message ?? "Unknown error"}`
      ...
      {extracted && (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <Card>
            ...
            <CardContent>
              <UtilizationGauge utilization={extracted.credit_utilization} />
            </CardContent>
          </Card>
          ...
          {loanTypes.length > 0 && (
            <Card>
              ...
              <CardContent>
                <DebtDistribution loanTypes={loanTypes} />
              </CardContent>
            </Card>
          )}
        </div>
